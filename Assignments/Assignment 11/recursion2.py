...
def countConsecutive(start,nums):
    count = 0
    num = nums[start]
...
def groupSumClump(start, nums, target):
    if (start >= len(nums)):
        ...
        numOcc = countConsecutive(start, nums)
        if numOcc > 1:
            return groupSumClump(start + numOcc, nums, target - nums[start] * numOcc) or \
                   groupSumClump(start + numOcc, nums, target)
        return groupSumClump(start + 1, nums, target - nums[start]) or \
...
def splitArray(nums):
    ...
    sum1 = 0
    sum2 = 0
    return splitArrayHelper(start, nums, sum1, sum2)
...
def splitArrayHelper(start, nums, sum1, sum2):
    ...
    if (start >= len(nums)):
        ...
        return splitArrayHelper(start + 1, nums, sum1 + nums[start], sum2) or \
                splitArrayHelper(start + 1, nums, sum1, sum2 + nums[start])
...
def splitOdd10(nums):
    ...
    sum1 = 0
    sum2 = 0
    return splitOdd10Helper(start, nums, sum1, sum2)
...
def splitOdd10Helper(start, nums, sum1, sum2):
    ...
    if start >= len(nums):
        ...
        return splitOdd10Helper(start + 1, nums, sum1 + nums[start], sum2) or \
...
def split53(nums):
    ...
    mult5 = 0
    mult3 = 0
    return split53Helper(start, nums, mult5, mult3)

def split53Helper(start, nums, mult5, mult3):
    ...
    if start >= len(nums):
        ...
        return split53Helper(start + 1, nums, mult5 + nums[start], mult3) or \
            split53Helper(start + 1, nums, mult5, mult3 + nums[start])
