...
def stringClean(str):
    ...
    if len(str) == 1:
        ...
        return stringClean(str[:-1]) + str[-1]

def countHix(str):
    if len(str) == 0:
        return 0
    elif str[-3:] == 'xhi':
        return 1 + countHix(str[:-1])
    else:
        return 0 + countHix(str[:-1])
...
#strCount but with overlapping for next function
def strCount2(str, sub):

    if str == '':
        return 0
    elif str[:len(sub)] == sub:
        return 1 + strCount2(str[1:], sub)
    else:
        return 0 + strCount2(str[1:], sub)
...
def strCopies(str, sub, n):
    ...
    if strCount2(str, sub) >= n:
        return True
    else:
        return False

def startAndEnd(str,sub):
    if str == '':
        return
    elif str[:len(sub)] != sub:
...
def strDist(str, sub):
    ...
    if startAndEnd(str, sub) == None:
        return 0
    else:
        return startAndEnd(str, sub)
